...
const commands = {
    help: () => {
        ...
        console.log('');
        console.log('  TEST SERVER:'.cyan.bold);
        console.log('  testlocal - Thử nghiệm kết nối server local'.white);
        console.log('  testhypixel - Thử nghiệm kết nối Hypixel Network'.white);
        console.log('  testminehut - Thử nghiệm kết nối Minehut hosting'.white);
        console.log('');
        console.log('  QUẢN LÝ NHIỀU BOT:'.cyan.bold);
        console.log('  create <số lượng> - Tạo ra số lượng bot chỉ định'.white);
        console.log('  connect <số> <host> [port] - Kết nối bot theo số thứ tự'.white);
        console.log('  disconnect <số> - Ngắt kết nối bot theo số thứ tự'.white);
        console.log('  chat <số> <tin nhắn> - Gửi tin nhắn từ bot cụ thể'.white);
        console.log('  remove <số> - Xóa và hủy bot theo số thứ tự'.white);
        console.log('  connectall <host> [port] - Kết nối toàn bộ bot tuần tự'.white);
        console.log('  connectfast <host> [port] - Kết nối nhanh 5 bot mỗi lần với proxy riêng'.white);
        console.log('  list - Liệt kê trạng thái của tất cả bot'.white);
        console.log('  chatall <tin nhắn> - Gửi tin nhắn đồng loạt từ mọi bot'.white);
        console.log('  spamall <tin nhắn> <lần> [delay] - Spam tin nhắn có chống phát hiện'.white);
        console.log('  testchat <tin nhắn> - Thử nghiệm gửi tin từ bot đầu tiên'.white);
        console.log('  forcechat <tin nhắn> - Ép buộc gửi tin bỏ qua mọi kiểm tra'.white);
        console.log('  checkchat - Kiểm tra khả năng chat của bot'.white);
        console.log('  tryperm - Thử nghiệm các lệnh quyền admin phổ biến'.white);
        console.log('  chatperm - Test chat với prefix để vượt qua hạn chế'.white);
        console.log('  removeall - Xóa toàn bộ bot khỏi hệ thống'.white);
        console.log('');
        console.log('  QUẢN LÝ PROXY:'.cyan.bold);
        console.log('  proxyadd <url> - Thêm proxy mới vào danh sách'.white);
        console.log('  proxyremove <url> - Loại bỏ proxy khỏi danh sách'.white);
        console.log('  proxylist - Hiển thị toàn bộ proxy và tình trạng'.white);
        console.log('  proxytest - Kiểm tra độ ổn định của tất cả proxy'.white);
        console.log('  proxyscrape [quick/full] - Thu thập proxy tự động từ internet'.white);
        console.log('  proxyon - Kích hoạt chế độ xoay proxy'.white);
        console.log('  proxyoff - Tắt chế độ xoay proxy'.white);
        console.log('');
        console.log('  HỆ THỐNG:'.cyan.bold);
        console.log('  core - Hiển thị số nhân CPU khả dụng'.white);
        console.log('  ram - Thông tin chi tiết về bộ nhớ RAM'.white);
        console.log('  workers - Trạng thái các worker thread đang chạy'.white);
        console.log('  quit/exit - Thoát và đóng chương trình'.white);
        console.log('');
        console.log('VÍ DỤ SỬ DỤNG:'.yellow.bold);
        console.log('  create 20'.gray + '                    # Tạo 20 bot với mỗi bot 1 proxy riêng');
        console.log('  connectfast cherry.pikamc.vn'.gray + ' # Kết nối 5 bot/lần, tự động register');
        console.log('  proxyscrape quick'.gray + '            # Thu thập proxy nhanh từ internet');
        console.log('  list'.gray + '                         # Kiểm tra tình trạng kết nối');
        console.log('  chatall Xin chào!'.gray + '           # Gửi tin nhắn từ tất cả bot');
        console.log('');
        console.log('🎯 QUY TRÌNH KHUYẾN NGHỊ:'.green.bold);
        console.log('  1. proxyscrape → 2. create → 3. connectfast → 4. chatall'.cyan);
        console.log('  📝 Mỗi bot sử dụng 1 proxy riêng, tự động xóa proxy khi bị ban'.yellow);
        console.log('');
    },
